...
export class OpenAIResponsesProxyAdapter implements UpstreamAdapter {
    ...
    public async handle(ctx: UpstreamRequestContext): Promise<void> {
        ...
        if (converted.warnings.length > 0) {
            Logger.warn(`OpenAI Responses 请求转换 warnings：${JSON.stringify(converted.warnings)}`);
        }
    ...
    private handleJsonResponse(
        ...
        try {
            ...
            if (converted.warnings.length > 0) {
                Logger.warn(`OpenAI Responses 响应转换 warnings：${JSON.stringify(converted.warnings)}`);
            }
