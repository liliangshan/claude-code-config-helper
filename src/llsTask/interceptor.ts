...
export function interceptAnthropicResponse(
    ...
    if (result.handledWorkflowTool) {
        ...
        // 导致任务流后续卡住不再续推。
        deps.autoContinueScheduler.resetMissingToolCounter('命中非任务流工具，本轮在实际执行');
        // 不立即续推：本应由 Claude Code 自己发 tool_result 往返继续干活。但部分
        // 情况下 CLI 这轮结束后不自动续做剩余工作，任务流卡在半截。改用空闲看门狗
        // 兜底：4 秒内无新请求则判定 CLI 已停，续推一次（详见 armIdleWatchdog）。
        deps.autoContinueScheduler.cancel('响应包含非任务流工具调用，等待 Claude Code 工具结果');
        deps.autoContinueScheduler.armIdleWatchdog();
